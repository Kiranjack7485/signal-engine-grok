...
async def analyze_coin(client, candidate, is_confirmation=False):
...
async def get_top_and_analyze(client):
    ...
    if not active:
        ...
        await check_pending_confirmations(client, window_name)
        return
    ...
    try:
        ...
        for t in tickers:
            symbol = t.get('symbol')
            if symbol not in FIXED_COINS: continue
        ...
        tasks = [analyze_coin(client, c, is_confirmation=False) for c in candidates]
...
async def check_pending_confirmations(client, window_name):
    now = datetime.utcnow()
    to_remove = []
    for symbol, pending in list(pending_setups.items()):
        ...
        conf = await analyze_coin(client, fake, is_confirmation=True)
        ...
        if conf['direction'] != pending['direction']:
            ...
            btc_data = await analyze_coin(client, {'symbol': 'BTCUSDT', 'change_24h_pct': 0, 'volume_usdt': 0}, is_confirmation=True)
            ...
            while retry_count < 5:
                try:
                    ...
                    if active:
                        ...
                        logger.info(f"Idle – outside trading windows ({window_name})")
                        await check_pending_confirmations(client, window_name)
                    break
                except Exception as e:
